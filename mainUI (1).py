...
class Ui_MainWindow:
    def setupUi(self, MainWindow):
        ...
        self.mode_selection_verticalLayoutWidget = QtWidgets.QWidget(self.mode_selection)
        self.mode_selection_verticalLayoutWidget.setGeometry(QtCore.QRect(10, 30, 71, 80))
        self.mode_selection_verticalLayoutWidget.setObjectName("mode_selection_verticalLayoutWidget")
        self.mode_verticalLayout = QtWidgets.QVBoxLayout(self.mode_selection_verticalLayoutWidget)
        self.mode_verticalLayout.setContentsMargins(0, 0, 0, 0)
        self.mode_verticalLayout.setObjectName("mode_verticalLayout")
        self.mean_button = QtWidgets.QRadioButton(self.mode_selection_verticalLayoutWidget)
        self.mean_button.setObjectName("mean_button")
        self.mode_verticalLayout.addWidget(self.mean_button)
        self.sum_button = QtWidgets.QRadioButton(self.mode_selection_verticalLayoutWidget)
        ...
        self.y_min_axis.setMinimum(-100)
        self.y_min_axis.setMaximum(100)
        self.y_min_axis.setValue(-50)
        ...
        self.y_max_axis_rms.setValue(50)
    ...
    def plotBlmGraph(self, x, y, xlbl="", ylbl="", lbl="line", min_time=None, max_time=None):
        ...
        if self.sum_all.isChecked():
            plot(x, self.sumBLMs(y), self.blmAx, lbl="BLM SUM", xmin=min_time, xmax=max_time,  ymin = -0.02,
                    ymax = 0.3)
        ...
        if self.R5IM.isChecked():
            plot(x, -y[-1], self.r5imax, lbl="R5IM", xmin=min_time, xmax=max_time,  ymin = -0.02,
                    ymax = 3.5,legend="upper left")
    ...
    def plotRMSgraph(self, y, rms):
        ...
        self.sumAx = plot(self.xrms, self.yrms, self.sumAx, True, "Time(s)",xmin=0,xmax=50, ymin=y_min, ymax=y_max)
        self.sumAx = plot(self.xrms, self.error_below, self.sumAx, False, "Time(s)",xmin=0,xmax=50, ymin =y_min, ymax=y_max)
        self.sumAx = plot(self.xrms, self.error_above, self.sumAx, False, "Time(s)",xmin=0,xmax=50, ymin =y_min, ymax=y_max)
        self.sumAx.fill_between(self.xrms,self.error_above,self.error_below,alpha=0.5, edgecolor='#CC4F1B', facecolor='#FF9848')
        self.canvas.draw()
